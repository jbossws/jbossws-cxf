...
arrive with URL-encoded paths (e.g., "/Caff%C3%A8"), but destinations are registered with decoded names by CXF.
The stack can automatically attempt to decode incoming request paths to match the registered endpoints.
...
Individual deployments can override the global setting using a Bus property in the `jboss-webservices.xml` descriptor:
