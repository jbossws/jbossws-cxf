...
=== URL Path Decoding for NLS Characters

JBossWS-CXF can automatically handle URL-encoded paths containing National Language Symbols (NLS) or other non-ASCII
characters. When a web service `name` or `serviceName` contains special characters (e.g., "Caffè"), HTTP requests
arrive with URL-encoded paths (e.g., "/Caff%C3%A8"), but destinations are by default registered with decoded names by
CXF.
The stack can automatically try to decode incoming request paths to match the registered endpoints.

This behavior is disabled by default and can be controlled globally or per-deployment.

==== System properties

- The `org.jboss.ws.cxf.decodeUrlPath` system property controls the default behavior across all deployments.
The default value is `false`.

- The `org.jboss.ws.cxf.urlCharset` system property controls the charset that will be used to decode the URL path.
The default value is `UTF-8`.

===== Note about Undertow URL encoding/decoding charset

The `org.jboss.ws.cxf.urlCharset` system property should be aligned with Undertow’s URL_CHARSET configuration option.

==== Global configuration

To enable URL path decoding globally, set the system property to `true` in the WildFly configuration:

....
<system-properties>
  <property name="org.jboss.ws.cxf.decodeUrlPath" value="true"/>
</system-properties>
....

==== Per-deployment configuration

Individual deployments can override the global setting using a deployment property in the `jboss-webservices.xml`
descriptor:

....
<webservices xmlns="http://www.jboss.com/xml/ns/javaee" version="1.2">
  <property>
    <name>org.jboss.ws.cxf.decodeUrlPath</name>
    <value>true</value>
  </property>
  <!-- This is optional, since it would default to "UTF-8" -->
  <property>
    <name>org.jboss.ws.cxf.urlCharset</name>
    <value>UTF-8</value>
  </property>
</webservices>
....

The deployment-level property takes precedence over the system property, allowing fine-grained control per deployment.
