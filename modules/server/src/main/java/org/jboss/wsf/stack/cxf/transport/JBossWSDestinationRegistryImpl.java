...
public class JBossWSDestinationRegistryImpl extends DestinationRegistryImpl
{
   ...
   public AbstractHTTPDestination getDestinationForPath(String path, boolean tryDecoding) {
      ...
      if (endpoint == null) {
         LOGGER.debug("Cannot find an endpoint for the current request and look up the " +
                 Constants.JBWS_CXF_URL_CHARSET + " deployment property, which is expected when not processing a request.");
      } else {
