...
<<_predefined_client_and_endpoint_configurations>> . Endpoint configuration are
...
associated to a JBossWS <<_predefined_client_and_endpoint_configurations>>
...
See <<_published_wsdl_customization>>.
...
The
https://github.com/jbossws/jbossws-spi/tree/main/src/main/resources/schemajbossws-jaxws-config_4_0.xsd[jbossws-jaxws-config
...
<<_advanced_user_guide>>  of the documentation for getting started.
...
provide property values for <<_component_element>>
...
<<_configuration_through_deployment_descriptor>>
...
current deployment. Further details available <<_configuration_through_deployment_descriptor>> .
...
interceptors within <<_predefined_client_and_endpoint_configurations>> by specifying a list of interceptor class names for the
...
adds an additional descriptor based approach to avoid requiring
modifications to the actual client/endpoint code. Users can declare
features within <<_predefined_client_and_endpoint_configurations>> by specifying a list of feature class names for the
...
a defined _endpoint configuration_ . <<_predefined_client_and_endpoint_configurations>>
...
<<_web_service_provider_implementation>>
.
...
The ActAs scenario is an extension of <<_a_basic_ws_trust_scenario>>. In this example the ActAs service calls the
...
The OnBehalfOf scenario is an extension of <<_a_basic_ws_trust_scenario>> . In this example the OnBehalfOf service calls the
ws-service on behalf of a user. There are only a couple of additions to
...
resolve services. When running in-container, a <<_jboss_modules>> dependency to the the `org.apache.cxf.impl` module is
